...
from routes.filme_routes import filme_routes             # DE routes.filme_routes IMPORTAR filme_routes: conjunto de rotas do módulo filme

app = Flask(__name__)                                    # Cria a aplicação Flask, passando __name__ para identificar o módulo principal
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///filmes.db'  # Configuração: URL de conexão com o banco de dados SQLite chamado filmes.db
...
app.register_blueprint(filme_routes)                     # Registra o blueprint das rotas de filme na aplicação
